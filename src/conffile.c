 ...
 */
static int
cf_uid_in_list(uid_t value, const uid_list *list)
{
    if(!list)
        return 0;
 ...
 */
static int
cf_gid_within_groups(gid_t gid)
{
    if(cf_get_groups() < 0) {
 ...
 */
static int
cf_user_has_groups(const gid_list *list)
{
    if(!list)
        return 0;
    for(unsigned i = 0; i < list->len; i++)
...
void
ci_bool_set_default(ci_bool *c, int val)
{
    c->def = val;
}

int
ci_bool_allowed(const ci_bool *c)
{
    if(c->def) {
...
void
ci_bool_dump(const ci_bool *c, FILE *out)
{
...
*/
static size_t
cf_spec_key_number(const cf_spec *spec)
{
    switch(spec->type) {
    case boolean_item:
        return 4;
...
*/
static void
cf_spec_prepare_keys(cf_spec *spec, cf_key *keys)
{
...
*/
static cf_key *
cf_spec_build_keys(cf_spec *specs)
{
    cf_spec *s = specs;
    cf_key *keys;
    cf_key *k;
...
*/
static cf_key *
cf_key_find(const char *key, cf_key *keys)
{
    while(keys->key) {
...
*/
static int
cf_read_line(FILE *file, char *dest, size_t nb)
{
    while(1) {
        int len;
...
*/
static regex_t comment_RE, declaration_RE, uint_RE, blank_RE, true_RE, false_RE;
 ...
 */
static int
cf_classify_line(char *line, char **name_ptr, char **value_ptr)
{
    regmatch_t m[3];
    if(!(*line) || !regexec(&comment_RE, line, 0, NULL, 0) ||
    ...
    if(!regexec(&declaration_RE, line, 3, m, 0)) {
        ...
        if(m[1].rm_so < 0) {
            fprintf(stderr, "There sure was a problem, it is in principle "
 ...
 */
static int
cf_get_boolean(const char *value, int *target)
{
    if(!regexec(&true_RE, value, 0, NULL, 0))
        ...
        *target = 0;
        ...
        fprintf(stderr,
                _("Error while reading configuration file: '%s' "
...
*/
static size_t
cf_count_chars(const char *str, const char *which)
{
    size_t val = 0;
    while(1) {
...
*/
static void
cf_trim_anew(const char *source, char *dest, size_t nb)
{
...
static int
cf_get_uid(const char *uid, uid_t *uid_r)
{
...
static int
cf_get_gid(const char *gid, gid_t *gid_r)
{
    char buffer[1024]; /* To trim the string */
 ...
 */
static int
cf_get_uidlist(char *value, uid_list *list)
{
 ...
 */
static int
cf_get_gidlist(char *value, gid_list *list)
{
    /* First, compute the number of commas in the list. */
    list->len = cf_count_chars(value, ",") + 1;
...
*/
static int
cf_read_stringlist(char *value, ci_string_list *target)
{
...
*/
static int
cf_key_assign_value(cf_key *key, char *value)
{
    ...
    case boolean_item: {
        ...
        case CF_KEY_INFO_NONE: { /* Normal */
            ...
            if(!cf_get_boolean(value, &val)) {
                ci_bool_set_default(t,
                                    val); /* Or directly use the internals ? */
                return 0;
            }
            return -1;
        }
...
int
cf_read_file(FILE *file, cf_spec *specs)
{
